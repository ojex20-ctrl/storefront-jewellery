...
async function main() {
  ...
  await prisma.adminUser.upsert({
    ...
    update: {},
    create: { email: "admin@syra.com", passwordHash: hash, name: "SYRA Admin", role: "superadmin" },
  })
  console.log("✓ Admin user: admin@syra.com / admin123")
  ...
  const products = [
    ...
    { name: "Luna Pearl Drop", slug: "syra-necklace-01", kind: "Necklace", caption: "A whisper of elegance.", price: 18500, metals: '["18k Gold"]', stones: '["Pearl"]', sizes: '[]', tag: "NEW", image: "https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&q=80&w=800", gallery: '["https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&q=80&w=800"]', description: "Ethically sourced pearls suspended from a delicate anti-tarnish gold chain. Waterproof and sweat-proof.", material: "Surgical Steel + 18k Gold PVD", warranty: "2 Year Anti-Tarnish Guarantee" },
  ...
  for (const p of products) {
    await prisma.product.upsert({ where: { slug: p.slug }, update: {}, create: { ...p, published: true, featured: true } })
  }
  ...
  for (let i = 0; i < sections.length; i++) {
    const s = sections[i]
    await prisma.siteContent.upsert({ where: { page_section: { page: s.page, section: s.section } }, update: {}, create: { ...s, published: true, sortOrder: i } })
  }
