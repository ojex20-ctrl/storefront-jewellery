  ...
  return (
    <div className="flex min-h-screen bg-[#F5F3EF] text-[#1A1A1C]">
      ...
      <main className="flex-1 p-8 md:p-12">
        <Link href="/admin/customers" className="text-xs uppercase tracking-widest text-[#1A1A1C]/50 hover:text-[#c9a36b]">← Customers</Link>
        <h1 className="mb-1 mt-3 font-display text-4xl tracking-tight">
          {`${customer.firstName} ${customer.lastName}`.trim() || customer.email}
        </h1>
        <p className="mb-8 font-mono text-xs text-[#1A1A1C]/50">{customer.email}</p>
        ...
        <div className="mb-10 overflow-x-auto border border-[#1A1A1C]/10 bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {orders.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-[#1A1A1C]/40">No orders yet.</td></tr>
              )}
              {orders.map((o) => (
                <tr key={o.id} className="border-t border-[#1A1A1C]/5">
                  ...
                  <td className="px-4 py-3 text-[#1A1A1C]/60">{o.createdAt.toLocaleDateString()}</td>
        ...
        {addresses.length === 0 ? (
          <p className="text-sm text-[#1A1A1C]/40">No saved addresses.</p>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {addresses.map((a) => (
              <div key={a.id} className="border border-[#1A1A1C]/10 bg-white p-5 text-sm">
                ...
                <p className="mt-1 font-mono text-xs text-[#1A1A1C]/50">{a.phone}</p>
              </div>
            ))}
          </div>
        )}
